...
export default function CreatorPostsIndexPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        setError(getErrorMessage(loadError, "Failed to load creator posts."));
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <CreatorPageShell
      title="Posts"
      description="Posts are attached to a specific experience. Choose an experience below to open its post feed and publish updates as the creator owner."
    >
      {loading ? (
        <PageCard>
        ...
        <PageCard>
          <p className="text-sm text-white/65">
            You need to create an experience first before publishing posts.
          </p>
        </PageCard>
      ) : (
