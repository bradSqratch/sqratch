...
5. Nulls `sourceShopDomain` wherever it equals the redacted domain, across **all** brands' `BrandRewardOffer` rows — not only the brand currently holding the domain, since an earlier relink can leave a different brand's rows referencing this domain as a historical `sourceShopDomain`. Rows are never deleted; a nulled `sourceShopDomain` requires review/reselection before that offer is considered current again.
